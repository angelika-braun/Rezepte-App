...
const Home = () => {
  const { API_URL, showDetails, setShowDetails, recipes, setRecipes, error, setError } = useContext(APIContext)
  ...
  return (
    ...
    {recipes.map((recipe) => (
        <section key={recipe.idMeal} className="content">
          ...
          <button className="readMoreBtn" onClick={() => setShowDetails(!showDetails)}>
          ...
          <div className="details">
            ...
            <ul>
              {recipe.strInstructions.split('\r\n').map((step, index) => {
                if (step.trim() !== '') {
                  return <li key={index} className="instructions">{step}</li>;
                }
                return null;
              })}
            </ul>
            ...
            {recipe.strYoutube ? (
              ...
              <p style={{ color: "red", margin: "20px" }}>Es ist kein Video verfügbar</p>
            )
            }
                        </div>
)}
